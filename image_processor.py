...
def resize_image(image, height, width,
    ...
    ### Seed so that results are always the same
    np.random.seed(44377)
        ...
        if width_ratio > height_ratio:
            ...
            if channels > 1:
                ...
                for i in range(channels):
                    noise_size_int.append(int(noise_size[i]))
                noise = np.random.randint(0, 255, noise_size_int).astype('uint8')
            else:
                noise = np.random.randint(0, 255, noise_array).astype('uint8')
